...
void Heap::balance(Node* root, bool print){
	...
	if(root->left != 0){
		...
		if(left_data > root_data){
			...
			root->data = left_data;
			if(print)
				cout << "Swapped the position of " << root_data << " and " << left_data << endl;
		}
	}
	if(root->right != 0){
		...
		if(right_data > root_data){
			...
			if(print)
				cout << "Swapped the position of " << root_data << " and " << right_data << endl;
		}
	}
...
void Heap::insert(int data, bool print){ //Determines whether or not we should print logs
	...
	if(insert_node == 0){
		if(print)
			cout << "Couldn't find a spot to insert" << endl;
		return;
	}
	...
	if(insert_node->left == 0){ //If the left side is open, insert there
		...
		num_nodes++;
		if(print)
			cout << "Inserted " << data << " to the left of " << insert_node->data << endl;
	...
	if(insert_node->right == 0){ //If the right side is open, insert there after checking the left
		...
		num_nodes++;
		if(print)
			cout << "Inserted " << data << " to the right of " << insert_node->data << endl;
		balance(insert_node, print);
		return;
	}
	delete new_node; //This means we couldn't insert it so we just delete the node
	if(print)
		cout << "Couldn't insert node" << endl;
}
